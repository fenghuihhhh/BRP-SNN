...
state_prediction=[0,1,2,3,4]
for m in range(25):
    train_state = np.array(GRF2(state[state_prediction[m%5]], new_par.n_state))
    train_sensory = np.zeros((new_par.n_sensory, new_par.T))
    if m==16:        
        train_sensory = np.array(GRF(2, new_par.n_prediction))
    if m==17:        
         train_sensory =np.array(GRF(2, new_par.n_prediction))
    if m==18:        
        train_sensory = np.array(GRF(2, new_par.n_prediction))
    if m==19:        
         train_sensory =np.array(GRF(2, new_par.n_prediction))
    if m>19:        
        train_state =np.zeros((new_par.n_state, new_par.T))
    # train_sensory = np.array(GRF(2, new_par.n_sensory))
    for x in layer2:
    ...
    #per 100ms fire_rate  
    spike_1=np.array(train_state)
    a=0
    for i in range(36):
        for j in range(100):
            if spike_1[i][j]==1:
                a+=1
    fire_rate_1.append(a)
    
    spike_2=np.array(spike_y)
    spike_2=spike_2[:,m:(m+1)]
    b=0
    for i in range(25):
        for j in range(1):
            if spike_2[i][j]==1:
                b+=1
    fire_rate_2.append(b)
    


fire_rate=[0]
for i in range(25):
    f=fire_rate_1[i]+fire_rate_2[i]
    fire_rate.append(f)
...
axes.set_ylim([0,15])
axes.set_xlim([0,26])
plt.plot(KK,fire_rate)
plt.legend()
plt.title('fire_rate_per100ms')
plt.show()
plt.savefig('fire_rate_per100ms')
